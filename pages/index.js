...
function LinkCard({ href, title, image }) {
  return (
    <a
      href={href}
      className="p-1 border border-gray-300 w-full mb-3 rounded-sm hover:scale-105 transition-all bg-white items-center"
    >
      <div className="flex w-full text-center">
        ...
        <h2 className="flex justify-center items-center text-gray-900 w-full -ml-10 font-semibold">
          {title}
        </h2>
      </div>
    </a>
    ...
    <>
      ...
      <div className="flex flex-col items-center h-screen mt-16 mx-8">
        <Image
          ...
          height={96}
        />
        <h1 className="font-semibold text-2xl mt-4 mb-8 text-white">{name}</h1>
        {links.map((link) => (
